...
std::vector<TrunkMessage> P25Parser::decode_mbt_data(unsigned long opcode, boost::dynamic_bitset<> &header, boost::dynamic_bitset<> &mbt_data, unsigned long sa, unsigned long nac, int sys_num) {
  ...
  if (opcode == 0x0) { // grp voice channel grant
    ...
    message.sys_id = syid;
    os << "mbt3a rfss status: syid: " << syid << " rfid " << rfid << " stid " << stid << " ch1 " << ch1 << "(" << channel_id_to_string(ch1, sys_num) << ")" << std::endl;
    message.meta = os.str();
    ...
    BOOST_LOG_TRIVIAL(trace) << "mbt04\tUnit to Unit Chan Grant\tChannel ID: " << std::setw(5) << ch << "\tFreq: " << f / 1000000.0 << "\tTarget ID: " << std::setw(7) << ta << "\tTDMA " << get_tdma_slot(ch, sys_num) << "\tSource ID: " << sa;
  } else {
  ...
  if (opcode == 0x00) { // group voice chan grant
    ...
    if (mfrid == 0x90) {
      ...
      if (f1 != f2) {
        ...
        os << "tsbk02\tGrant Update\tChannel ID: " << std::setw(5) << ch2 << "\tFreq: " << FormatFreq(f2) << "\tga " << std::setw(7) << ga2 << "\tTDMA " << get_tdma_slot(ch2, sys_num);
        message.meta = os.str();
        //BOOST_LOG_TRIVIAL(trace) << os;
      }
      os << "tsbk02\tGrant Update\tChannel ID: " << std::setw(5) << ch1 << "\tFreq: " << f1 / 1000000.0 << "\tga " << std::setw(7) << ga1 << "\tTDMA " << get_tdma_slot(ch1, sys_num);
      message.meta = os.str();
      //BOOST_LOG_TRIVIAL(trace) << os;
    }
    ...
    if (mfrid == 0x90) { // MOT_GRG_CN_GRANT_UPDT
      ...
      if (f1 != f2) {
        ...
        os << "MOT_GRG_CN_GRANT_UPDT(0x03): \tChannel ID: " << std::setw(5) << ch2 << "\tFreq: " << FormatFreq(f2) << "\tsg " << std::setw(7) << sg2 << "\tTDMA " << get_tdma_slot(ch2, sys_num);
        message.meta = os.str();
        //BOOST_LOG_TRIVIAL(trace) << os;
      }
      os << "MOT_GRG_CN_GRANT_UPDT(0x03): \tChannel ID: " << std::setw(5) << ch1 << "\tFreq: " << FormatFreq(f1) << "\tsg " << std::setw(7) << sg1 << "\tTDMA " << get_tdma_slot(ch1, sys_num);
      message.meta = os.str();
      //BOOST_LOG_TRIVIAL(trace) << os;
    } else {
      bool emergency = (bool)bitset_shift_mask(tsbk, 72, 0x80);
    ...
    os << "tsbk04\tUnit to Unit Chan Grant\tChannel ID: " << std::setw(5) << ch << "\tFreq: " << FormatFreq(f) << "\tTarget ID: " << std::setw(7) << ta << "\tTDMA " << get_tdma_slot(ch, sys_num) << "\tSource ID: " << sa;
    message.meta = os.str();
    ...
    message.emergency = emergency;
    message.encrypted = encrypted;
    os << "tsbk06\tUnit to Unit Chan Update\tChannel ID: " << std::setw(5) << ch << "\tFreq: " << f / 1000000.0 << "\tTarget ID: " << std::setw(7) << ta << "\tTDMA " << get_tdma_slot(ch, sys_num) << "\tSource ID: " << sa;
    message.meta = os.str();
    ...
    BOOST_LOG_TRIVIAL(trace) << "tsbk24"
    ...
    BOOST_LOG_TRIVIAL(trace) << "tsbk29 secondary cc: rfid " << std::dec << rfid << " stid " << stid << " ch1 " << ch1 << "(" << channel_id_to_string(ch1, sys_num) << ") ch2 " << ch2 << "(" << channel_id_to_string(ch2, sys_num) << ") ";
    ...
    message.message_type = SYSID;
    message.sys_id = syid;
    os << "tsbk3a rfss status: syid: " << syid << " rfid " << rfid << " stid " << stid << " ch1 " << chan << "(" << channel_id_to_string(chan, sys_num) << ")" << std::endl;
    message.meta = os.str();
    //BOOST_LOG_TRIVIAL(trace) << os;
  } else if (opcode == 0x3b) { // network status
    ...
    os << "tsbk3b net stat: wacn " << std::dec << wacn << " syid " << syid << " ch1 " << ch1 << "(" << channel_id_to_string(ch1, sys_num) << ") ";
    message.meta = os.str();
  } else if (opcode == 0x3c) { // adjacent status
    unsigned long rfid = bitset_shift_mask(tsbk, 48, 0xff);
    unsigned long stid = bitset_shift_mask(tsbk, 40, 0xff);
    ...
    Channel temp_chan = {
        ...
        bw * .125};
    add_channel(iden, temp_chan, sys_num);
    os << "tsbk3d iden id " << std::dec << iden << " toff " << toff * 0.25 << " spac " << spac * 0.125 << " freq " << freq * 0.000005;
    message.meta = os.str();
  } else {
